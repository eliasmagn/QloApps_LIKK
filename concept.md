...
- Operations automation now lives in the `kloperations` module, which seeds run/task tables, generates housekeeping and maintenance tasks via cron, emails daily digests/reminders and provides an Operations → Tasks console with manual task authoring plus CSV/ICS exports for staff updates.
...
6. **Operations automation** – the module now generates housekeeping and disable-driven maintenance tasks, mails daily digests plus overdue reminders, ships CSV/ICS exports per [`docs/blueprints/operations-automation.md`](docs/blueprints/operations-automation.md), and supports manual task authoring from the Operations console; upcoming work focuses on assignment workflows.
