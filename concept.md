...
- Quote payloads now feed a TCPDF-based `QuotePdfGenerator` so staff can export branded PDFs directly from persisted `KLQuote` records; the inquiry board sidebar lists stored quotes and offers permission-aware download/email actions for quick guest follow-up.
- The inquiry board sidebar now renders stored quotes as cards with status badges, approval/decline transitions, audit-note logging and the existing PDF/email actions so coordinators can manage the entire quote lifecycle without leaving the board.
