...
6. **Operations automation** – the first increment ships with automated housekeeping task generation and an admin console (`kloperations` module); the next milestone extends the same module with maintenance task types, notification digests and CSV/ICS exports per [`docs/blueprints/operations-automation.md`](docs/blueprints/operations-automation.md).
