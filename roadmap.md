...
- ✅ **Quote review inspector** – persisted quotes now render in the inquiry sidebar with status badges, approve/decline transitions that log audit notes, and direct PDF/email controls so coordinators can manage proposals without leaving the board (see [`devtasks/inquiry-quote-review.task.md`](devtasks/inquiry-quote-review.task.md)).
- ✅ **Quote email workflow** – guest delivery now runs through `KLQuoteMailDispatcher`, which attaches PDFs, embeds share-token download links, honours configurable sender/reply-to settings and ships the `kl_quote_notification.*` templates surfaced via the inquiry board (see [`devtasks/quote-email-workflow.task.md`](devtasks/quote-email-workflow.task.md)).
...
- ✅ **Rate plans & packages** – introduce configurable price plans (BAR, corporate, residency programmes) plus bundled packages for weekly ateliers, catering-inclusive stays, etc., as laid out in [`docs/blueprints/rate-plans-packages.md`](docs/blueprints/rate-plans-packages.md). Base database tables (`kl_rate_plan*`, `kl_package*`, `kl_quote`) and ObjectModel scaffolding now ship with the module to unblock admin UIs and pricing services. The back office exposes both **Rate Plans** and **Packages** tabs so staff can manage plan metadata, eligibility scopes, seasonal adjustments and assemble bundled offers via the new component builder, and the `KLQuotePricingEngine` now generates inquiry-ready quote payloads with seasonal adjustments and package components.
  - ✅ Persisted quotes now feed a TCPDF-powered `QuotePdfGenerator`, letting staff download or email branded PDFs straight from the inquiry board sidebar while regression tests pin the document hash for sample data.
- 🚧 **Frontend storytelling** – the home page now renders the presenter-driven storytelling landing across residencies, ateliers, gastronomy and programme sections; the next milestone keeps deepening the dedicated resource templates. A new blueprint ([`docs/blueprints/front-office-storytelling.md`](docs/blueprints/front-office-storytelling.md)) maps the presenter/services layer, CMS slots and rollout plan while implementation tasks stay tracked in [`devtasks/front-office-storytelling.task.md`](devtasks/front-office-storytelling.task.md).
- ✅ Scaffolding complete: the Hotel General Settings storytelling toggle (persisted as `WK_STORYTELLING_ENABLED` with `_KUNSTORT_STORYTELLING_LAUNCH_` still honoured as a legacy seeding flag) gates a new `ResidenciesController`, `HotelReservationSystemStorytellingPresenter` aggregates taxonomy/packages/CMS data and `themes/hotel-reservation-theme/storytelling/residencies.tpl` renders placeholder sections ready for editorial content.
...
- ✅ **ICS/CSV exports** – the Operations console can now export filtered task slices (date windows, statuses, resource kinds, teams) in CSV and ICS formats for downstream scheduling, embedding the active filters into filenames and calendar metadata.
