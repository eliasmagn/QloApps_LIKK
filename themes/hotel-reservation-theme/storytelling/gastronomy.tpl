  ...
  <div class="kl-storytelling kl-storytelling--gastronomy">
    <section class="kl-storytelling__hero">
      <div class="kl-storytelling__hero-copy">
        ...
        {else}
          <h1 class="h1">{l s='Dining & gastronomy at Kunstort Lehnin' d='Shop.Theme.Kunstort'}</h1>
          <p class="lead text-muted">
            {l s='Discover the kitchens, refectories and communal dining spaces that keep residencies nourished. The sections below surface taxonomy-backed profiles while the editorial team prepares long-form storytelling copy.' d='Shop.Theme.Kunstort'}
          </p>
        {/if}
      </div>
      ...
      {if $storytelling.sections}
        {foreach from=$storytelling.sections item=section}
          <article id="{$section.anchor|escape:'html':'UTF-8'}" class="kl-storytelling__section">
                ...
                {foreach from=$section.profiles item=profile}
                  <div class="kl-storytelling__profile">
                    ...
                    {if $profile.amenities}
                      <ul class="list-unstyled kl-storytelling__profile-amenities">
            ...
            {else}
              <p class="text-muted">{l s='Gastronomy profiles will appear here once storytelling entries are published.' d='Shop.Theme.Kunstort'}</p>
            {/if}
          </article>
        {/foreach}
      {else}
        <p class="text-muted">{l s='Catering storytelling is being prepared and will launch once profiles are published.' d='Shop.Theme.Kunstort'}</p>
      {/if}
    </section>
    ...
    {if isset($storytelling.packages) && $storytelling.packages}
      <section class="kl-storytelling__packages">
        <header class="kl-storytelling__section-header">
          <h2 class="h2">{l s='Featured dining programmes' d='Shop.Theme.Kunstort'}</h2>
          <p class="text-muted">{l s='These packages highlight catering-led residencies once editors flag them for promotion.' d='Shop.Theme.Kunstort'}</p>
        </header>
