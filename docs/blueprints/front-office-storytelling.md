...
1. **Scaffolding (Milestone Kick-off)**
   ...
   - Ship an admin-controlled toggle under *Hotel Reservation System → General Settings* (persisted as `WK_STORYTELLING_ENABLED` with `_KUNSTORT_STORYTELLING_LAUNCH_` still recognised as a legacy seeding flag) so rollout can be staged without editing `config/defines_custom.inc.php`.
