...
- ✨ **Storytelling spotlight** – a gradient ribbon beneath the header highlights weekly programme releases and links straight to the storytelling overview and inquiry form, replacing the legacy search widget.
...
- 🧶 **Storytelling scaffolding** – feature-flagged residencies (`/residencies`), studios (`/studios`), gastronomy (`/gastronomy`) and programme (`/programme`) landings now pull taxonomy-driven sections, scope-aware featured package groups, grouped availability snapshots, amenity callouts, slot-level inquiry CTAs and CMS-managed hero/highlight/practical/FAQ/testimonial slots when the **Storytelling landing pages** toggle is enabled under *Hotel Reservation System → General Settings → Website Configuration* (legacy `/index.php?controller=…` URLs remain compatible).
- 🧾 **Storytelling metadata** – storytelling controllers now assign translated meta titles, descriptions, canonical URLs and breadcrumbs while the theme renders canonical/Open Graph tags so residencies, ateliers, gastronomy and programme pages share consistent previews and sitemap signals.
- 🪄 **Shared storytelling layout** – all storytelling templates (including the optional home landing) render through a `storytelling-layout-slim` partial that enforces a 70rem canvas with responsive side padding so typography and spacing align across resource kinds.
...
- 🧾 **Quote review workflow** – the inquiry sidebar renders saved quotes as actionable cards with status badges, approve/decline transitions that log audit notes, and the same PDF/email tooling so coordinators can manage the full quote lifecycle without leaving the board.
...
Use the **Storytelling landing pages** toggle exposed in *Hotel Reservation System → General Settings → Website Configuration* to expose the storytelling landings at `/residencies`, `/studios`, `/gastronomy` and `/programme` (the historical `/index.php?controller=…` URLs continue to redirect). Each page is powered by `HotelReservationSystemStorytellingPresenter`, which aggregates taxonomy-driven sections with metadata, surfaces featured `KLPackage` entries flagged for promotion, caches grouped availability highlights, hydrates CMS-managed copy slots and now feeds amenity callouts alongside capacity summaries. Availability snapshots are flushed automatically via `KLStoryAvailabilityCache` whenever bookings, inquiries or room disable ranges mutate so the next front-office request rebuilds a fresh payload while cron-driven booking syncs keep the dataset current. Installer/upgrade hooks now refresh dispatcher routes and regenerate rewrite rules so the clean URLs resolve immediately after deployment.

### Toggle storytelling on or off

1. Navigate to **Hotel Reservation System → General Settings** in the back office.
...
- A new `Storytelling spotlight` ribbon sits below the nav, highlighting current programme releases and inquiry actions with responsive CTAs that stack on mobile.
...
- Storytelling landings capture their markup and feed it into the shared `storytelling-layout-slim` partial so every page benefits from the same max-width, padding and typography rhythm.
